...
async def attempt_connection(sunlogin, method = 1, *args):
    ...
    if res != "ok":
        _LOGGER.debug("Cloud API get_devices_list failed: %s", res)
        return {"reason": "device_list_failed", "msg": res}

    _LOGGER.debug("Cloud API connection succeeded.")
...
class HejiaqinConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        errors = {}
        placeholders = {}
        _LOGGER.debug("start async_step_user")
...
#            error, resp = await async_get_devices_list(self.hass, api_key)
            if error is None:
                ...
                if len(devices) > 0:
                    ...
                    api_key = rapi_key
                    _LOGGER.debug("Get ApiKey ret: " + rapi_key)
                    self.device_list = devices
                    return await self._create_entry(user_input)                    
                _LOGGER.debug("msg: unknown")
                errors["base"] = 'unknown'
                placeholders = {"msg": "no devices"}
            else:
                _LOGGER.debug("msg: " + error)
                errors["base"] = 'unknown'
                placeholders = {"msg": error}

        _LOGGER.debug("end async_step_user")
